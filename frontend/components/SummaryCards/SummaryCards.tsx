...
export const SummaryCards: React.FC<SummaryCardsProps> = ({ transactions }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <Card title = "Total Balance" amount = {balance} icon = {<Wallet className="text-primary w-6 h-6" />} colorClass="text-primary" />
      <Card title = "Total Income" amount = {income}  icon = {<ArrowUpCircle className="text-emerald-600 w-6 h-6" />} colorClass="text-emerald-600" />
      <Card title = "Total Expense" amount = {expense} icon = {<ArrowDownCircle className="text-red-500 w-6 h-6" />} colorClass="text-red-500" />
    </div>
  );
};
