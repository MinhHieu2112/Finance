...
export const detectCurrencyFromAmountInput = (rawValue: string | number | null | undefined): Currency | null => {
  const normalizedValue = String(rawValue ?? '').trim();
  if (!normalizedValue) {
    return null;
  }

  const matchedCurrency = CURRENCY_MATCHERS.find(({ pattern }) => pattern.test(normalizedValue));
  return matchedCurrency?.currency || null;
};

export const parseCurrencyAmountInput = (rawValue: string | number | null | undefined) => {
  const normalizedValue = String(rawValue ?? '').trim();
  const detectedCurrency = detectCurrencyFromAmountInput(normalizedValue);
